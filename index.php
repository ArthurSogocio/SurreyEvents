...
//This is because if they return home after getting a callback URL from trying to access their watchlist, they are not concerned with getting immediately redirected to their bookmarks if they choose to log in normally after.
        ...
        <table>
            <tr class="main-content">
                <td>
                    ...
                    <ul>
                        <?php
                        //Image and caption for featured event on homepage.
                        $imagesrc = "assets/placeholder.png";
                        $imagecap = "";

                        while ($r = mysqli_fetch_assoc($result)) {
                            if ($imagecap == "") { //Performs the following only once (i.e. shows the first result as the featured event).
                                $imagesrc = $r['img_url'];
                    ...
                    if (isset($_SESSION['valid_user'])) {
                        //Selects most selected category in user's bookmarks.
                        $mostquery = "SELECT COUNT(categories.name) as total, categories.id "
                                ...
                                . "WHERE members.user_id = " . $_SESSION['valid_user'] . " GROUP BY categories.name ORDER BY total DESC LIMIT 1 ";
                        ...
                        if (mysqli_num_rows($mostresult) != 0) {
                            ...
                            //Creates the recommendation list if recommendations were successfully returned.
                            if ($recresult) {                       
                            ?>
                            <h1>Recommended</h1>
                            <?php
                ...
                <!-- Featured event image and caption. -->
                <td style="width: 50%;">
